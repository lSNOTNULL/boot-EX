...
import org.example.bootex.dto.GeminiRequestDTO;
import org.example.bootex.dto.GeminiResponseDTO;
...
public class GeminiService {
    ...
    public String generate(String text) throws IOException, InterruptedException {
        ...
        HttpRequest request =
        HttpRequest.newBuilder(URI.create(url)).POST(
...
//        return response.body();
        GeminiResponseDTO geminiResponseDTO = objectMapper.readValue(response.body(), GeminiResponseDTO.class);
        return geminiResponseDTO.candidates().get(0).content().parts().get(0).text();
    }
}
